...
def draw_predict_result_label(image, truth_label, truth_mask, probability_label):
    ...
    for c in range(4):
        ...
        draw_shadow_text(overlay, 'pos%d %d %0.2f %s  ' % (c + 1, truth_label[c], probability_label[c], asterisk),
                         (5, (c + 1) * 30), 1, color[c + 1], 2)
...
def draw_predict_result_label_one_defect_only(image, truth_label, truth_mask, probability_label):
    ...
    if 1:
        ...
        draw_shadow_text(overlay, 'pos%d %d %0.2f %s  ' % (1, truth_label, probability_label, asterisk), (5, (1) * 30),
                         1, color[1], 2)
    ...
    for c in [1, 2, 3, 4]:
        for i in range(length):
            ...
            if (p > 0.5) or (t > 0.5):
                clr = color[c]
            if i == 0:
                draw_shadow_text(overlay1, 'pos%d [%d] %0.2f %s  ' % (c, t, p, asterisk), (5 + i * w, (c) * 30), 1, clr,
                                 2)
            else:
...
def draw_predict_result_softmax_label(image, truth_label, truth_mask, probability_label):
    ...
    for c in [1, 2, 3, 4]:
        ...
        draw_shadow_text(overlay, 'pos%d %d %0.2f %s  ' % (c, truth_label[c - 1], probability_label[c], asterisk),
                         (5, (c + 1) * 30), 1, color[c], 2)
