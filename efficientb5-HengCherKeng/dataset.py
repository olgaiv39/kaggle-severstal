...
def run_check_train_dataset():
    ...
    dataset = SteelDataset(
        mode="train", csv=["train.csv"], split=["train_b0_11568.npy"], augment=None  #
    )
    print(dataset)
    # exit(0)
...
def run_check_augment():
    ...
    for b in range(len(dataset)):
        ...
        if 1:
            for i in range(100):
                image1, label1, mask1, infor = augment(
                    image.copy(), label.copy(), mask.copy(), infor
                )
