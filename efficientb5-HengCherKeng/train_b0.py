            ...
            if 0:
                for di in range(3):
                    if (iter + di) % 1000 == 0:
                        ...
                        probability_attention = (
                            probability_attention.data.cpu().numpy().reshape(-1, 4, 5)
                        )
                        truth_label = truth_label.data.cpu().numpy()
                        truth_mask = truth_mask.data.cpu().numpy()
                        truth_attention = (
                            truth_attention.data.cpu().numpy().reshape(-1, 4, 5)
                        )

                        image = input_to_image(input)
                        for b in range(batch_size):
                            image_id = infor[b].image_id
                            result = draw_predict_result_8cls(
                                image[b],
                                truth_label[b],
                                truth_mask[b],
                                truth_attention[b],
                                probability_attention[b],
                            )

                            image_show("result", result, resize=0.5)
                            cv2.imwrite(
                                out_dir + "/train/%05d.png" % (di * 100 + b), result
                            )
                            cv2.waitKey(1)
                            pass
        pass  # -- end of one data loader --
