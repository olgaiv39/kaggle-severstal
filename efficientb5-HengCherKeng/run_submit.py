    ...
    for t, (input, truth_label, truth_mask, truth_attention, infor) in enumerate(
        ...
        with torch.no_grad():
            ...
            if "flip_lr" in augment:
                logit = torch.nn.parallel.data_parallel(
                    net, torch.flip(input, dims=[3])
                )
        ...
        probability_mask = (probability_mask.data.cpu().numpy() * 255).astype(
            numpy.uint8
        )
        probability_label = (probability_label.data.cpu().numpy() * 255).astype(
            numpy.uint8
        )
...
def run_submit_segmentation():
    out_dir = "/root/share/project/kaggle/2019/steel/result15/efficientb5-fpn-crop256x400-foldb1"
    initial_checkpoint = "/root/share/project/kaggle/2019/steel/result15/efficientb5-fpn-crop256x400-foldb1/checkpoint/00053000_model.pth"
    train_split = ["valid_b1_1000.npy"]
    out_dir = "/root/share/project/kaggle/2019/steel/result15/efficientb5-fpn-crop256x400-foldb0h"
    initial_checkpoint = "/root/share/project/kaggle/2019/steel/result15/efficientb5-fpn-crop256x400-foldb0h/checkpoint/00051000_model.pth"
    ...
    if mode == "train":
        test_dataset = SteelDataset(
            mode="train", csv=["train.csv"], split=train_split, augment=None
        )
    if mode == "test":
        test_dataset = SteelDataset(
            mode="test",
    ...
    if 0:
        # Save
        image_id, truth_label, truth_mask, probability_label, probability_mask, = do_evaluate_segmentation(
            net, test_dataset, augment
        )
        if 1:
            # Save
            write_list_to_file(out_dir + "/submit/%s/image_id.txt" % (mode), image_id)
            numpy.savez_compressed(
                out_dir + "/submit/%s/truth_label.uint8.npz" % (mode), truth_label
            )
            numpy.savez_compressed(
                out_dir + "/submit/%s/truth_mask.uint8.npz" % (mode), truth_mask
            )
            numpy.savez_compressed(
                out_dir + "/submit/%s/probability_label.uint8.npz" % (mode),
                probability_label,
            )
            numpy.savez_compressed(
                out_dir + "/submit/%s/probability_mask.uint8.npz" % (mode),
                probability_mask,
            )
    if 1:
        image_id = read_list_from_file(out_dir + "/submit/%s/image_id.txt" % (mode))
        truth_label = numpy.load(out_dir + "/submit/%s/truth_label.uint8.npz" % (mode))[
            "arr_0"
        ]
        truth_mask = numpy.load(out_dir + "/submit/%s/truth_mask.uint8.npz" % (mode))[
            "arr_0"
        ]
        probability_label = numpy.load(
            out_dir + "/submit/%s/probability_label.uint8.npz" % (mode)
        )["arr_0"]
        probability_mask = numpy.load(
            out_dir + "/submit/%s/probability_mask.uint8.npz" % (mode)
        )["arr_0"]
    if 1:
        # Decode
        ...
        index = numpy.ones((num_test, 4, 256, 1600), numpy.uint8) * numpy.array(
            [1, 2, 3, 4], numpy.uint8
        ).reshape(1, 4, 1, 1)
    ...
    log.write("\n")
    if mode == "train":
        predict_label = probability_label > (numpy.array(threshold_label) * 255).astype(
            numpy.uint8
        ).reshape(1, 4)
        predict_mask = probability_mask > (
            numpy.array(threshold_mask_pixel) * 255
        ).astype(numpy.uint8).reshape(1, 4, 1, 1)
    ...
    if mode == "test":
        ...
        df = pd.DataFrame(
            zip(image_id_class_id, encoded_pixel),
